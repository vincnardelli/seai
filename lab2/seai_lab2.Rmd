---
title: "SEAI 2022 - R - Lab 2 - Intro to R"
author: "Vincenzo Nardelli - vincnardelli@gmail.com - https://github.com/vincnardelli"
output:
  html_document:
    df_print: paged
---
...
A ``data.frame`` is an analog of a matrix which can contains a differenc class of object for each column. It is the perfect class to store the data which we commonly use for our analysis.
...
Now we will explore the use of dplyr in subsetting, manipulating and summarising our data frame.
Most of the task that we will cover are possible also in base R without the use of others packages but dplyr simplify a lot the work. 
...
```
As you can see the select function is nested in the filter function to keep the order we decided.
Using pipes the code could be writed as:
```{r}
titanic %>% 
...
You can think a pipe as the command "then".
The previous algorithm can be interpreted as:
- Load data then
- Select Id and Age then
- Filter Age less then 18
